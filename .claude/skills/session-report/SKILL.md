...
3. **Ensure the output directory exists and copy the template:**
   ```sh
   mkdir -p <your-output-dir> && cp ~/.claude/plugins/marketplaces/claude-plugins-official/plugins/session-report/skills/session-report/template.html <your-output-dir>/session-report-$(date +%Y%m%d-%H%M).html
   ```
